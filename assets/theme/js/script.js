!function() {
    ...
    if (!document.getElementById('top-1')) {
        ...
        e.id = "top-1";
        e.className = "engine";
        e.innerHTML = '<a href="https://mobirise.me">Mobirise</a> Mobirise v4.6.6';
...
(function($){
    ...
    $(document).find('.panel-group').find('.panel-heading').each(function(target) {
      $(this).click(function() {
        var spanItem = $(this).children('span');
        if($(spanItem).hasClass('pseudoMinus') ) {
          $(spanItem).closest('.panel').removeClass('colorBG');  
          ...
          $('.panel-group').find('.signSpan').each(function() {
            ...
            $(this).closest('.panel').removeClass('colorBG');
          });
          $(spanItem).removeClass('pseudoPlus').addClass('pseudoMinus');
          $(spanItem).closest('.panel').addClass('colorBG');
    ...
    $(document).find('.toggleStyles').find('.toggle-heading').each(function(target) { 
      $(this).click(function() {
        var spanItem = $(this).children('.toggle-heading span');
        if($(spanItem).hasClass('pseudoMinus')) {
          $(spanItem).closest('.toggle-panel').removeClass('colorBG');
          $(spanItem).removeClass('pseudoMinus').addClass('pseudoPlus').parent().css('border', '');
        } else {
          $(spanItem).closest('.toggle-panel').addClass('colorBG'); 
    ...
    // Form2 checker
     var $form2 = $(document).find('.form2'); 

     if($form2.find('.checkbox').css('display') && $form2.find('.checkbox input').prop('checked', false)){
        $form2.find('button[type="submit"]').prop('disabled', true);
     };

     $form2.find('.checkbox').on('click', function() {
        if($(this).children('input').is(":checked")){
            $form2.find('button[type="submit"]').prop('disabled', false);
            $form2.find('.checkbox label').addClass('checked');
         } else {
            $form2.find('button[type="submit"]').prop('disabled', true);
            $form2.find('.checkbox label').removeClass('checked');
         };
     });
